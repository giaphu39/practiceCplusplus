...
bool AddAfterallX(LIST& l, NODE* newNODE, int x){
    // bị lỗi do chỉ có 1 NDOE nhưng gán cho nhiều NODE -> muốn gán hết sau x thì mỗi lần phải tạo NODE mới
    NODE* p = l.pHead;
    bool flag = 0;
    while( p != NULL){
        if(p->info == x){
            ...
            break; // chỉ 1 lần đầu có số trùng x mới thêm vào
            // cải tiến bằng cách không có tham số newNODE truyền vào mà là giá trị y mỗi lần gặp là khởi tạo NODE mới rồi add thêm vào
...
void Function(NODE* p){
    ...
    while(p!=NULL){
        ...
        while(j!=NULL){
            if(p->info > j->info){
                output(p->info,j->info);
                break;
            }
            j = j->pNext;
        }
        if(j == NULL) output(p->info, -1);
...
void Remove_k_th(LIST& l,int k){
    ...
    int i =1;
    while(p != NULL){
        // trường hợp đặc biệt phần tử cần xóa ở đầu
        if(i == k-1){
            int temp;
            RemoveAfterQ(l,p,temp);
        }
