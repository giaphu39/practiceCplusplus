...
int Delete(Hashtable &ht, int maso, int &nprob) {
    ...
    int ind;
    while(true){
        if(nprob >= ht.M) return 0;
        ind = ((maso % ht.M) + nprob*nprob) % ht.M;
        if(ht.table[ind].Maso == maso){
            ht.table[ind].Maso = DELETE; // do in ra lieen quan toi ma so, chu y phai dung bien cho san
             ht.n -- ;//  phải đặt ở đây chứ
            return 1;
        }
        nprob++;
    }

    // ht.n -- ; // nho insert va delete co tac dong so phan tu
