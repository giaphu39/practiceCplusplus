...
TNODE* searchNode(TREE T, int x) {
    if (T != NULL) {
         if ((T->pLeft != NULL && T->pLeft->key == x) || // dòng này chứng tỏ tìm node cha
...
void FindSiblings(TREE T,int x)
{
    ...
    if(level==0){
        cout<<T->key<<" is Root.";return ;
    }
    TNODE*a=searchNode(T,x);
    if(a->pLeft==NULL && a->pRight!=NULL)
    {
        cout<<a->pRight->key<<" has no siblings.";
    }
    else if(a->pRight==NULL && a->pLeft!=NULL)
    {
        cout<<a->pLeft->key<<" has no siblings.";
    }
    else {
        cout<<a->pLeft->key<<" and "<<a->pRight->key<<" are siblings.";
    }
}
