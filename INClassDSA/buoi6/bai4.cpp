...
void Fun(LIST& l){
    ...
    while(p != NULL){
        temp = p->pNext;
        while(temp != NULL){
            // 3 trường hợp: ở chung ox, ở chung oy, không ở chung thằng nào
            if (p->info.ox + temp->info.ox == 0 && p->info.oy + temp->info.oy == 0)
            { 
                if(p->info.ox == temp->info.ox && p->info.oy == temp->info.oy){
                    continue;
                }
                flag =1;
            ...
            // else if(p->info.ox == temp->info.ox && (p->info.oy + temp->info.oy ==0) && p->info.oy != temp->info.oy){ 
            //     flag =1;
            //     output1(p->info.ox, p->info.oy);
            //     output1(temp->info.ox, temp->info.oy);
            //     cout<<endl;
            ...
            // else if(p->info.oy == temp->info.oy && (p->info.ox + temp->info.ox ==0) && p->info.ox != temp->info.ox){ 
            //     flag =1;
            //     output1(p->info.ox, p->info.oy);
            //     output1(temp->info.ox, temp->info.oy);
            //     cout<<endl;
